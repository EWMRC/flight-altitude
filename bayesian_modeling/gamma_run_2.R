...
inits <- function(){list(mu_bias = rnorm(1,0,1),
                         ...
                         scale_flight = rtruncnorm(n = 1, a = 0, mean = 300, sd = 200))}

parameters <- c("mu_bias", "sigma_error", "shape_flight", "scale_flight") #
...
m_test <- jags(data=jags_data, inits=inits, parameters.to.save = parameters, 
               model.file=here("bayesian_modeling", "gamma_model_2.jags"), n.chains=nc, n.iter=ni, n.burnin=nb,
               parallel=T)
