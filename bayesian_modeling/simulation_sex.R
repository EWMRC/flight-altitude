...
known_ground <- 13000
unknown_flight_m <- 110
unknown_flight_f <- 110
unknown_ground <- 220
#nsim <- known_ground + unknown_flight + known_flight + unknown_ground

known_ground_df <- tibble(HAT = rnorm(n=known_ground, mean=0, sd=10), 
                          HAT_index = rep(1, known_ground),
                          sex = rep(0, known_ground))
unknown_ground_df <- tibble(HAT = rnorm(n=unknown_ground, mean=0, sd=10), 
                            HAT_index = rep(NA, unknown_ground),
                            sex = rep(0, unknown_ground))

unknown_flight_m_df <- tibble(HAT = rtruncnorm(n=unknown_flight_m, mean=330, sd=250, a = 0), 
                            HAT_index = rep(NA, unknown_flight_m),
                            sex = rep(0, unknown_flight_m))
unknown_flight_m_df$HAT <- map(unknown_flight_m_df$HAT, function(x){
  rnorm(n=1, mean = x, sd=10)
}) %>% 
  unlist()

unknown_flight_f_df <- tibble(HAT = rtruncnorm(n=unknown_flight_f, mean=330, sd=250, a = 0), 
                              HAT_index = rep(NA, unknown_flight_f),
                              sex = rep(1, unknown_flight_f))
unknown_flight_f_df$HAT <- map(unknown_flight_f_df$HAT, function(x){
  rnorm(n=1, mean = x, sd=10)
}) %>% 
  unlist()

dt <- bind_rows(known_ground_df, unknown_ground_df, unknown_flight_m_df, unknown_flight_f_df)
...
sink(here("bayesian_modeling", "simulation_sex.jags"))
cat("model{
   # likelihood
    for (i in 1:n_obs){
...
parameters <- c("mu_bias", "sigma_error", "mu_flight_intercept", "mu_flight_beta", "sigma_flight_intercept", "sigma_flight_beta") #
...
m_test <- jags(data=jags_data, inits=inits, parameters.to.save = parameters, 
               model.file=here("bayesian_modeling", "simulation_sex.jags"), n.chains=nc, n.iter=ni, n.burnin=nb,
               parallel=T)
