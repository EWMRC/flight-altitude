...
covariates <- read.csv(here("imported_movebank_data.csv"))

raw_data <- st_read(here("raw_elevation_values.shp")) %>% 
  st_drop_geometry() %>% 
  arrange(Field1) %>% 
...
parameters <- c("mu_bias", "sigma_error", "shape_flight", "rate_flight", "HAT_index") #
...
m_test <- jags(data=jags_data, inits=inits, parameters.to.save = parameters, 
               model.file=here("gamma_model.jags"), n.chains=nc, n.iter=ni, n.burnin=nb,
               parallel=T)

saveRDS(m_test, file = here("gamma_original_cluster.rds"))
