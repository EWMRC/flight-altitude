...
raw_data <- st_read(here("intermediate_files", "shapefile_3D_projected_ExportFeatures_2026.shp")) %>% 
  st_drop_geometry() %>% 
  arrange(Field1) %>% 
  dplyr::select(t_hae_m_1)

raw_data <- covariates %>% 
  bind_cols(raw_data)

raw_data <- rename(raw_data, t_hae_m = t_hae_m_1)
...
raw_data <- raw_data %>% 
  ...
  filter(!is.na(time_lubr))#discarding 188...or 1055 in March 2026
...
altitude_data %>%
  ...
  pull(height_above_terrain) %>%
  hist(main = "Possible flight locations")  # seems like a lot of possible flight in the negative altitudes...

# Scale locations between -1 and 1
altitude_data <- altitude_data %>% 
  mutate(hat_scaled = height_above_terrain/2183.475)  # change to 2183.495?
