...
# mean is shape/rate
mean_altitude_adult <- (age_results$sims.list$shape_flight[, 1]/age_results$sims.list$rate_flight[,1])*2183.475
mean(mean_altitude_adult)
sd(mean_altitude_adult)
quantile(mean_altitude_adult, c(.025,.975))

mean_altitude_juv <- (age_results$sims.list$shape_flight[, 2]/age_results$sims.list$rate_flight[,2])*2183.475
mean(mean_altitude_juv)
sd(mean_altitude_juv)
quantile(mean_altitude_juv, c(.025,.975))
...
sd_altitude_adult <- ((age_results$sims.list$shape_flight[, 1]/((age_results$sims.list$rate_flight[, 1])^2))^0.5)*2183.475
mean(sd_altitude_adult)
sd(sd_altitude_adult)
quantile(sd_altitude_adult, c(.025,.975))

sd_altitude_juv <- ((age_results$sims.list$shape_flight[, 2]/((age_results$sims.list$rate_flight[, 2])^2))^0.5)*2183.475
mean(sd_altitude_juv)
sd(sd_altitude_juv)
quantile(sd_altitude_juv, c(.025,.975))
...
set.seed(8)
draws_sampled_adult <- sample(1:length(age_results$sims.list$shape_flight[,1]), 
                             size = 10000)

results_shape_rate_adult <- map(draws_sampled_adult, #
                               function(index){
                                 res <- rgamma(n = 1000, 
                                               shape = age_results$sims.list$shape_flight[index,1],
                                               rate = age_results$sims.list$rate_flight[index,1]) %>% 
...
set.seed(8)
draws_sampled_juv <- sample(1:length(age_results$sims.list$shape_flight[,2]), 
                               size = 10000)

results_shape_rate_juv <- map(draws_sampled_juv, #
                                 function(index){
                                   res <- rgamma(n = 1000, 
                                                 shape = age_results$sims.list$shape_flight[index,2],
                                                 rate = age_results$sims.list$rate_flight[index,2]) %>% 
                                     density(n = 200, from = 0, to = 1)
...
ggsave(plot = plot_shape_rate_age_combined, 
       filename = here("graph_results", "plot_shape_rate_age.png"),
       width = 7/1.5,
       height = 6/1.5)
