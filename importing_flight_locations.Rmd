...
amwo_data <- amwo_data %>%
  transmute(ID = local_identifier,
            ...
            designated_before = !is.na(bas_confidence))
...
coastline_polygon <- st_read(here("coastline", "coastline", "ne_10m_coastline_poly.shp")) %>%
  st_transform(4326) %>%
...
## code to identify birds over the Great Lakes ####
# amwo_range <- readRDS("states.provinces.EPSG.4326.rds") %>% 
#   transmute(on_land = TRUE)
# amwo_sf3 <- amwo_sf %>%
#   st_join(amwo_range)
# amwo_sf3 <- amwo_sf3 %>%
...
# amwo_sf3 %>%
  # filter(on_land == FALSE) -> amwo_overwater
# write.csv(amwo_overwater, here("intermediate_files", "amwo_greatlakes.csv"))  


###these overwater exceptions are points confirmed to be overwater, NOT on land, hence the change in code.
overwater_exceptions <- read.csv(here("overwater_exceptions.csv"))

amwo_sf <- amwo_sf %>%
  mutate(on_land = if_else(event_id %in% overwater_exceptions$event_id, FALSE, on_land))

# set na values to TRUE (ie onland == FALSE is an overwater location, these locations have been proofed)
amwo_sf <- amwo_sf %>%
  mutate(on_land = if_else(is.na(on_land), TRUE, on_land))
...
# map the overwater locations: good for proofing the locations - currently unhappy
amwo_sf %>%
  filter(on_land == FALSE) %>% 
  st_as_sf(coords = c("lon", "lat"), crs = 4326) -> amwo_overwater
# write.csv(amwo_overwater, here("intermediate_files", "amwo_potential_overwater.csv"))

mapview::mapView(amwo_overwater)
