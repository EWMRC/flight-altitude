---
title: "R Notebook"
output: html_notebook
---

```{r}
library(tidyverse)
library(jagsUI)
library(here)
library(furrr)
library(bayestestR)
```

Fall is index 1, spring 2

```{r}
season_results <- readRDS(here("modeling", "results", "lnorm_season.rds"))
```

Median
```{r}
#fall
plan(multisession)

median_draws_fall <- future_map2(.x = rstan::extract(season_results, "mu_alt_fall")[[1]], 
            .y = rstan::extract(season_results, "sigma_alt_fall")[[1]],
            .progress = TRUE,
            .options = furrr_options(seed = 8),
            .f = function(x, y){
              res <- rlnorm(n = 10000, 
                          meanlog = x,
                          sdlog = y)
              
              res <- res * 2183.475
              
              return(median(res))
            })

plan(sequential)

median_draws_fall %>% 
  unlist() %>% 
  median() #205.1177

median_draws_fall %>% 
  unlist() %>% 
  hdi(ci = 0.5) #50% HDI: [179.01, 233.08]

median_draws_fall %>% 
  unlist() %>% 
  hdi(ci = 0.95) #95% HDI: [128.98, 279.66]

# spring
plan(multisession)

median_draws_spring <- future_map2(.x = rstan::extract(season_results, "mu_alt_spring")[[1]], 
            .y = rstan::extract(season_results, "sigma_alt_spring")[[1]],
            .progress = TRUE,
            .options = furrr_options(seed = 8),
            .f = function(x, y){
              res <- rlnorm(n = 10000, 
                          meanlog = x,
                          sdlog = y)
              
              res <- res * 2183.475
              
              return(median(res))
            })

plan(sequential)

median_draws_spring %>% 
  unlist() %>% 
  median() #284.9893

median_draws_spring %>% 
  unlist() %>% 
  hdi(ci = 0.5) #50% HDI: [245.01, 322.74]

median_draws_spring %>% 
  unlist() %>% 
  hdi(ci = 0.95) #95% HDI: [167.79, 393.43]
```

Mean
```{r}
#fall
plan(multisession)

mean_draws_fall <- future_map2(.x = rstan::extract(season_results, "mu_alt_fall")[[1]], 
            .y = rstan::extract(season_results, "sigma_alt_fall")[[1]],
            .progress = TRUE,
            .options = furrr_options(seed = 8),
            .f = function(x, y){
              res <- rlnorm(n = 10000, 
                          meanlog = x,
                          sdlog = y)
              
              res <- res * 2183.475
              
              return(mean(res))
            })

plan(sequential)

mean_draws_fall %>% 
  unlist() %>% 
  median() #319.594

mean_draws_fall %>% 
  unlist() %>% 
  hdi(ci = 0.5) #50% HDI: [283.29, 344.41]

mean_draws_fall %>% 
  unlist() %>% 
  hdi(ci = 0.95) #95% HDI: [234.25, 417.51]

#spring
plan(multisession)

mean_draws_spring <- future_map2(.x = rstan::extract(season_results, "mu_alt_spring")[[1]], 
            .y = rstan::extract(season_results, "sigma_alt_spring")[[1]],
            .progress = TRUE,
            .options = furrr_options(seed = 8),
            .f = function(x, y){
              res <- rlnorm(n = 10000, 
                          meanlog = x,
                          sdlog = y)
              
              res <- res * 2183.475
              
              return(mean(res))
            })

plan(sequential)

mean_draws_spring %>% 
  unlist() %>% 
  median() #446.9399

mean_draws_spring %>% 
  unlist() %>% 
  hdi(ci = 0.5) #50% HDI: [392.80, 486.25]

mean_draws_spring %>% 
  unlist() %>% 
  hdi(ci = 0.95) #95% HDI: [315.32, 603.03]
```

Are these birds regularly flying lower than the flight heights reported via radar?
Calculate the percentage of these locations that fall below the minimum	threshold in Horton et al. 2016 (155m)
```{r}
#fall
plan(multisession)

pct_below_radar_threshold_fall <- future_map2(.x = rstan::extract(season_results, "mu_alt_fall")[[1]], 
            .y = rstan::extract(season_results, "sigma_alt_fall")[[1]],
            .progress = TRUE,
            .options = furrr_options(seed = 8),
            .f = function(x, y){
              res <- rlnorm(n = 10000, 
                          meanlog = x,
                          sdlog = y)
              
              res <- res * 2183.475
              
              return(sum(res < 155)/10000)
            })

plan(sequential)

pct_below_radar_threshold_fall %>% 
  unlist() %>% 
  median() #0.3822

pct_below_radar_threshold_fall %>% 
  unlist() %>% 
  hdi(ci = 0.5) #50% HDI: [0.32, 0.44]

pct_below_radar_threshold_fall %>% 
  unlist() %>% 
  hdi(ci = 0.95) #95% HDI: [0.21, 0.55]

#spring
plan(multisession)

pct_below_radar_threshold_spring <- future_map2(.x = rstan::extract(season_results, "mu_alt_spring")[[1]], 
            .y = rstan::extract(season_results, "sigma_alt_spring")[[1]],
            .progress = TRUE,
            .options = furrr_options(seed = 8),
            .f = function(x, y){
              res <- rlnorm(n = 10000, 
                          meanlog = x,
                          sdlog = y)
              
              res <- res * 2183.475
              
              return(sum(res < 155)/10000)
            })

plan(sequential)

pct_below_radar_threshold_spring %>% 
  unlist() %>% 
  median() #0.2585

pct_below_radar_threshold_spring %>% 
  unlist() %>% 
  hdi(ci = 0.5) #50% HDI: [0.19, 0.31]

pct_below_radar_threshold_spring %>% 
  unlist() %>% 
  hdi(ci = 0.95) #95% HDI: [0.10, 0.45]
```

How may of these locations are at an altitude where they might collide with a building (0-47m)?
```{r}
#fall
plan(multisession)

pct_below_building_threshold_fall <- future_map2(.x = rstan::extract(season_results, "mu_alt_fall")[[1]], 
            .y = rstan::extract(season_results, "sigma_alt_fall")[[1]],
            .progress = TRUE,
            .options = furrr_options(seed = 8),
            .f = function(x, y){
              res <- rlnorm(n = 10000, 
                          meanlog = x,
                          sdlog = y)
              
              res <- res * 2183.475
              
              derived <- sum(res <= 47)/10000
              
              return(derived)
            })

plan(sequential)

pct_below_building_threshold_fall %>% 
  unlist() %>% 
  median() #0.0576

pct_below_building_threshold_fall %>% 
  unlist() %>% 
  hdi(ci = 0.5) #50% HDI: [0.01, 0.06]

pct_below_building_threshold_fall %>% 
  unlist() %>% 
  hdi(ci = 0.95) #95% HDI: [0.00, 0.16]

#spring
plan(multisession)

pct_below_building_threshold_spring <- future_map2(.x = rstan::extract(season_results, "mu_alt_spring")[[1]], 
            .y = rstan::extract(season_results, "sigma_alt_spring")[[1]],
            .progress = TRUE,
            .options = furrr_options(seed = 8),
            .f = function(x, y){
              res <- rlnorm(n = 10000, 
                          meanlog = x,
                          sdlog = y)
              
              res <- res * 2183.475
              
              derived <- sum(res <= 47)/10000
              
              return(derived)
            })

plan(sequential)

pct_below_building_threshold_spring %>% 
  unlist() %>% 
  median() #0.02795

pct_below_building_threshold_spring %>% 
  unlist() %>% 
  hdi(ci = 0.5) #50% HDI: [0.00, 0.03]

pct_below_building_threshold_spring %>% 
  unlist() %>% 
  hdi(ci = 0.95) #95% HDI: [0.00, 0.12]
```

How may of these locations are at an altitude where they might collide with a comm tower (0-305m)?
```{r}
#fall
plan(multisession)

pct_below_comm_tower_threshold_fall <- future_map2(.x = rstan::extract(season_results, "mu_alt_fall")[[1]], 
            .y = rstan::extract(season_results, "sigma_alt_fall")[[1]],
            .progress = TRUE,
            .options = furrr_options(seed = 8),
            .f = function(x, y){
              res <- rlnorm(n = 10000, 
                          meanlog = x,
                          sdlog = y)
              
              res <- res * 2183.475
              
              derived <- sum(res <= 305)/10000
              
              return(derived)
            })

plan(sequential)

pct_below_comm_tower_threshold_fall %>% 
  unlist() %>% 
  median() #0.6644

pct_below_comm_tower_threshold_fall %>% 
  unlist() %>% 
  hdi(ci = 0.5) #50% HDI: [0.63, 0.71]

pct_below_comm_tower_threshold_fall %>% 
  unlist() %>% 
  hdi(ci = 0.95) #95% HDI: [0.54, 0.78]

#spring
plan(multisession)

pct_below_comm_tower_threshold_spring <- future_map2(.x = rstan::extract(season_results, "mu_alt_spring")[[1]], 
            .y = rstan::extract(season_results, "sigma_alt_spring")[[1]],
            .progress = TRUE,
            .options = furrr_options(seed = 8),
            .f = function(x, y){
              res <- rlnorm(n = 10000, 
                          meanlog = x,
                          sdlog = y)
              
              res <- res * 2183.475
              
              derived <- sum(res <= 305)/10000
              
              return(derived)
            })

plan(sequential)

pct_below_comm_tower_threshold_spring %>% 
  unlist() %>% 
  median() #0.5286

pct_below_comm_tower_threshold_spring %>% 
  unlist() %>% 
  hdi(ci = 0.5) #50% HDI: [0.47, 0.58]

pct_below_comm_tower_threshold_spring %>% 
  unlist() %>% 
  hdi(ci = 0.95) #95% HDI: [0.38, 0.69]
```

How may of these locations are at an altitude where they might collide with a wind turbine (32.2-163.8m)?
```{r}
#fall
plan(multisession)

pct_below_wind_threshold_fall <- future_map2(.x = rstan::extract(season_results, "mu_alt_fall")[[1]], 
            .y = rstan::extract(season_results, "sigma_alt_fall")[[1]],
            .progress = TRUE,
            .options = furrr_options(seed = 8),
            .f = function(x, y){
              res <- rlnorm(n = 10000, 
                          meanlog = x,
                          sdlog = y)
              
              res <- res * 2183.475
              
              derived <- sum(res > 32.2 & res <= 163.8)/10000
              
              return(derived)
            })

plan(sequential)

pct_below_wind_threshold_fall %>% 
  unlist() %>% 
  median() #0.3778

pct_below_wind_threshold_fall %>% 
  unlist() %>% 
  hdi(ci = 0.5) #50% HDI: [0.35, 0.44]

pct_below_wind_threshold_fall %>% 
  unlist() %>% 
  hdi(ci = 0.95) #95% HDI: [0.24, 0.48]

#spring
plan(multisession)

pct_below_wind_threshold_spring <- future_map2(.x = rstan::extract(season_results, "mu_alt_spring")[[1]], 
            .y = rstan::extract(season_results, "sigma_alt_spring")[[1]],
            .progress = TRUE,
            .options = furrr_options(seed = 8),
            .f = function(x, y){
              res <- rlnorm(n = 10000, 
                          meanlog = x,
                          sdlog = y)
              
              res <- res * 2183.475
              
              derived <- sum(res > 32.2 & res <= 163.8)/10000
              
              return(derived)
            })

plan(sequential)

pct_below_wind_threshold_spring %>% 
  unlist() %>% 
  median() #0.2665

pct_below_wind_threshold_spring %>% 
  unlist() %>% 
  hdi(ci = 0.5) #50% HDI: [0.22, 0.32]

pct_below_wind_threshold_spring %>% 
  unlist() %>% 
  hdi(ci = 0.95) #95% HDI: [0.13, 0.40]
```

Quantify the likelihood that one seasonal mean is greater than the other
This is called probability of superiority: see "A Probability-Based Measure of Effect Size: Robustness to Base Rates and Other Factors"
```{r}
sum(unlist(mean_draws_spring) > unlist(mean_draws_fall))*100/length(unlist(mean_draws_spring))
#93.97333% probability that the spring mean is higher than the fall mean
```

